...
class ResponseGenerator:
    ...
    def _base_response(
        self, response_type: str, message: str, priority: str, actions=None
    ):
    ...
    def _moderate_response(self):
        ...
        return self._base_response(
            ...
            "MEDIUM",
            ["Continue emotional monitoring", "Encourage supportive communication"],
        )
    ...
    def _contains_crisis_language(self, message: str) -> bool:
    ...
    def _select_response(
        self, risk_level: str, decision: Dict[str, Any], last_message: str
    ):
        ...
        if (
            ...
            or signals.get("critical_crisis", False)
            or self._contains_crisis_language(last_message)
        ):
    ...
    def generate(self, decision: Dict, last_message: str = "") -> Dict:

        decision = decision or {}

        risk_level = decision.get("final_risk_level", "Safe")

        response = self._select_response(risk_level, decision, last_message)

        response.update(
            {
                "risk_level": risk_level,
                "risk_score": round(decision.get("final_risk_score", 0), 4),
                "safety_note": (
