...
class ConversationPatternAnalyzer:
    ...
    def _detect_escalation(
        ...
        if (
            ...
            (
                "negative_emotion" in stages
                and
                "social_isolation" in stages
            )
    ...
    def analyze(
        ...
        isolation_count = self._count_patterns(
            messages,
            self.isolation_keywords
        )


        hopelessness_count = self._count_patterns(
            messages,
            self.hopelessness_keywords
        )


        negative_count = self._count_patterns(
            messages,
            self.negative_keywords
        )


        crisis_count = self._count_patterns(
            messages,
            self.crisis_keywords
        )
        ...
        result = {
            ...
            "isolation_score":
                ...
                round(
                    min(
                        isolation_count / total_messages,
                        1
                    ),
                    4
                ),
            ...
            "hopelessness_score":
                ...
                round(
                    min(
                        hopelessness_count / total_messages,
                        1
                    ),
                    4
                ),
        ...
        if result["negative_language_score"] >= 0.5:
            ...
            risk_indicators.append(
                "Frequent negative language detected"
            )
