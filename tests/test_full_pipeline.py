...
def test_home_endpoint():
    ...
    assert "Mental Health Safety Analyzer" in data["message"]
...
def test_single_message_pipeline_structure():
    ...
    payload = {"text": "I feel hopeless and nobody understands me."}
...
def test_single_message_distress_detection():
    ...
    payload = {"text": "I feel hopeless and very sad"}
...
def test_high_risk_conversation_escalation():
    ...
    response = client.post("/analyze-conversation", json=payload)
    ...
    assert signals["crisis_signal"] or signals["critical_crisis"]
...
def test_crisis_response_generation():
    ...
    assert len(safety["message"]) > 20

    assert safety["response_type"] in ["safety_support", "crisis_support"]

    assert safety["priority"] in ["HIGH", "CRITICAL"]
...
def test_xai_output_quality():
    ...
    assert len(xai["summary"]) > 10

    assert "reasons" in xai or "key_factors" in xai
